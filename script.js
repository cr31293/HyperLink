var attacks = [
    {
        name: 'Jab 1',
        image: 'gifs/nrml/jabOne.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "5%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "4",
                shieldStun: "4",
                shieldAdv: "-9",
                size: "4.4"
            },
            {
                hitboxId: "0",
                damage: "5%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "4",
                shieldStun: "4",
                shieldAdv: "-9",
                size: "4"
            },
            {
                hitboxId: "0",
                damage: "5%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "4",
                shieldStun: "4",
                shieldAdv: "-9",
                size: "2.4"
            },
            {
                hitboxId: "0",
                damage: "5%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "4",
                shieldStun: "4",
                shieldAdv: "-9",
                size: "2.4"
            },
        ]
    },
    {
        name: 'Jab 2',
        image: 'gifs/nrml/jabTwo.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "3%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "-7",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "3%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "-7",
                size: "4"
            },
            {
                hitboxId: "2",
                damage: "3%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "-7",
                size: "2.4"
            },
            {
                hitboxId: "3",
                damage: "3%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "-7",
                size: "2.4"
            },
        ]
    },
    {
        name: 'Jab3',
        image: 'gifs/nrml/jabThree.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "6%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "5",
                shieldStun: "4",
                shieldAdv: "-21",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "6%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "5",
                shieldStun: "4",
                shieldAdv: "-21",
                size: "4.4"
            },
            {
                hitboxId: "2",
                damage: "6%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "5",
                shieldStun: "4",
                shieldAdv: "-21",
                size: "2.8"
            },
            {
                hitboxId: "3",
                damage: "5%",
                angle: "361°",
                kbg: "60",
                bkb: "10",
                skb: "0",
                hitLag: "4",
                shieldStun: "4",
                shieldAdv: "-9",
                size: "2"
            },
        ]
    },
    {
        name: 'Rapid Jab',
        image: 'gifs/nrml/rapidJab.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "1%",
                angle: "0°",
                kbg: "40",
                bkb: "15",
                skb: "0",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: "-10",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "1%",
                angle: "0°",
                kbg: "40",
                bkb: "15",
                skb: "0",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: "-10",
                size: "4.4"
            },
            {
                hitboxId: "2",
                damage: "1%",
                angle: "0°",
                kbg: "40",
                bkb: "15",
                skb: "0",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: "-10",
                size: "2.8"
            },
            {
                hitboxId: "3",
                damage: "1%",
                angle: "0°",
                kbg: "40",
                bkb: "15",
                skb: "0",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: "-10",
                size: "2"
            },
        ]
    },
    {
        name: 'dashATK',
        image: 'gifs/nrml/dashAtk.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "12%",
                angle: "80°",
                kbg: "100",
                bkb: "10",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "-26/-21",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "11%",
                angle: "70°",
                kbg: "100",
                bkb: "10",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-27/-21",
                size: "2.8"
            },
            {
                hitboxId: "2",
                damage: "11%",
                angle: "60°",
                kbg: "100",
                bkb: "10",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-27/-22",
                size: "2"
            },
            {
                hitboxId: "3",
                damage: "9%",
                angle: "90°",
                kbg: "100",
                bkb: "10",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-27/-22",
                size: "4.4"
            },
        ]
    },
    {
        name: "U Smash",
        image: 'gifs/nrml/uSmash.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "First Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "4",
                angle: "98°",
                kbg: "100",
                bkb: "40",
                skb: "26",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "-",
                size: "3.2"
            },
            {
                hitboxId: "1",
                damage: "4",
                angle: "98°",
                kbg: "100",
                bkb: "40",
                skb: "30",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "-",
                size: "3.2"
            },
            {
                hitboxId: "2",
                damage: "4",
                angle: "98°",
                kbg: "100",
                bkb: "40",
                skb: "33",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "-",
                size: "3.2"
            },
            {
                hitboxId: "3",
                damage: "4",
                angle: "98°",
                kbg: "100",
                bkb: "40",
                skb: "33",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "-",
                size: "2"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Second Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "2",
                angle: "90°",
                kbg: "100",
                bkb: "0",
                skb: "30",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: "-",
                size: "3.2"
            },
            {
                hitboxId: "1",
                damage: "2",
                angle: "90°",
                kbg: "100",
                bkb: "0",
                skb: "33",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: "-",
                size: "3.2"
            },
            {
                hitboxId: "2",
                damage: "2",
                angle: "90°",
                kbg: "100",
                bkb: "0",
                skb: "37",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: "-",
                size: "3.2"
            },
            {
                hitboxId: "3",
                damage: "2",
                angle: "90°",
                kbg: "100",
                bkb: "0",
                skb: "40",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: "-",
                size: "2"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Third Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "10%",
                angle: "90°",
                kbg: "80",
                bkb: "90",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-4/-2",
                size: "3.2"
            },
            {
                hitboxId: "1",
                damage: "10%",
                angle: "90°",
                kbg: "80",
                bkb: "90",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-4/-2",
                size: "3.6"
            },
            {
                hitboxId: "2",
                damage: "9%",
                angle: "90°",
                kbg: "80",
                bkb: "90",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-4/-2",
                size: "3.2"
            },
            {
                hitboxId: "3",
                damage: "9%",
                angle: "90°",
                kbg: "80",
                bkb: "90",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-4/-2",
                size: "2"
            },
        ]
    },
    {
        name: "U Smash",
        image: 'gifs/nrml/dSmash.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "First Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "13%",
                angle: "75°",
                kbg: "90",
                bkb: "26",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "-25/-23",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "16%",
                angle: "75°",
                kbg: "90",
                bkb: "26",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "-23/-21",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "17",
                angle: "75°",
                kbg: "90",
                bkb: "26",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "-23/-21",
                size: "2.8"
            },
            {
                hitboxId: "3",
                damage: "16%",
                angle: "75°",
                kbg: "90",
                bkb: "26",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "-23/-21",
                size: "3.2"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Second Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "11%",
                angle: "75°",
                kbg: "90",
                bkb: "20",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-15/-13",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "16%",
                angle: "75°",
                kbg: "90",
                bkb: "20",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "-12/-10",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "17%",
                angle: "75°",
                kbg: "90",
                bkb: "20",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "-12/-10",
                size: "2.8"
            },
            {
                hitboxId: "3",
                damage: "16%",
                angle: "75°",
                kbg: "90",
                bkb: "20",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "-12/-10",
                size: "3.2"
            }
        ]
    },
    {
        name: 'F Smash 1',
        image: 'gifs/nrml/fSmash1.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "13%",
                angle: "361°",
                kbg: "83",
                bkb: "30",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "-26/-23",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "14%",
                angle: "361°",
                kbg: "83",
                bkb: "30",
                skb: "0",
                hitLag: "7",
                shieldStun: "8",
                shieldAdv: "-25/-22",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "14%",
                angle: "361°",
                kbg: "83",
                bkb: "30",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "-25/-22",
                size: "2.8"
            },
            {
                hitboxId: "3",
                damage: "14%",
                angle: "361°",
                kbg: "83",
                bkb: "30",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "-25/-22",
                size: "5"
            }
        ]
    },
    {
        name: 'F Smash 2',
        image: 'gifs/nrml/fSmash2.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "18%",
                angle: "65°",
                kbg: "100",
                bkb: "30",
                skb: "0",
                hitLag: "9",
                shieldStun: "10",
                shieldAdv: "-28",
                size: "3.2"
            },
            {
                hitboxId: "1",
                damage: "20%",
                angle: "65°",
                kbg: "100",
                bkb: "30",
                skb: "0",
                hitLag: "9",
                shieldStun: "10",
                shieldAdv: "-28",
                size: "4"
            },
            {
                hitboxId: "2",
                damage: "20%",
                angle: "65°",
                kbg: "100",
                bkb: "30",
                skb: "0",
                hitLag: "9",
                shieldStun: "10",
                shieldAdv: "-28",
                size: "2.8"
            },
            {
                hitboxId: "3",
                damage: "20%",
                angle: "65°",
                kbg: "100",
                bkb: "30",
                skb: "0",
                hitLag: "9",
                shieldStun: "10",
                shieldAdv: "-28",
                size: "4"
            }
        ]
    },
    {
        name: 'U Tilt',
        image: 'gifs/nrml/uTilt.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "9%",
                angle: "95°",
                kbg: "122",
                bkb: "30",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-14/-9",
                size: "3.6"
            },
            
            {
                hitboxId: "1",
                damage: "9%",
                angle: "85°",
                kbg: "130",
                bkb: "30",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-14/-9",
                size: "4.2"
            },
            {
                hitboxId: "2",
                damage: "9%",
                angle: "85°",
                kbg: "124",
                bkb: "30",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-14/-9",
                size: "2.8"
            },
            {
                hitboxId: "3",
                damage: "9%",
                angle: "85°",
                kbg: "123",
                bkb: "30",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-14/-9",
                size: "2.8"
            }
        ]
    },
    {
        name: 'D Tilt',
        image: 'gifs/nrml/dTilt.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "11%",
                angle: "80°",
                kbg: "50",
                bkb: "80",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-12/-10",
                size: "4.4"
            },
            
            {
                hitboxId: "1",
                damage: "11%",
                angle: "280°",
                kbg: "50",
                bkb: "80",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-12/-10",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "11%",
                angle: "280°",
                kbg: "50",
                bkb: "80",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-12/-10",
                size: "3.6"
            },
            {
                hitboxId: "3",
                damage: "11%",
                angle: "280°",
                kbg: "50",
                bkb: "80",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-12/-10",
                size: "3.2"
            }
        ]
    },
    {
        name: 'F Tilt',
        image: 'gifs/nrml/fTilt.gif',
        type: 'normal',
        stats: [
            {
                hitboxId: "0",
                damage: "14%",
                angle: "361°",
                kbg: "90",
                bkb: "5",
                skb: "0",
                hitLag: "7",
                shieldStun: "8",
                shieldAdv: "-15/-12",
                size: "4.4"
            },
            
            {
                hitboxId: "1",
                damage: "15%",
                angle: "361°",
                kbg: "90",
                bkb: "5",
                skb: "0",
                hitLag: "8",
                shieldStun: "8",
                shieldAdv: "-15/-12",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "14%",
                angle: "361°",
                kbg: "90",
                bkb: "5",
                skb: "0",
                hitLag: "7",
                shieldStun: "8",
                shieldAdv: "-15/-12",
                size: "2.8"
            },
            {
                hitboxId: "3",
                damage: "13%",
                angle: "361°",
                kbg: "90",
                bkb: "2",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "-16/-13",
                size: "2"
            }
        ]
    },
    {
        name: 'N-air',
        image: 'gifs/aerial/nAir.gif',
        type: 'aerial',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Strong Hit (4-5)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "9%",
                angle: "361°",
                kbg: "100",
                bkb: "15",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-1",
                size: "4.088"
            },
            {
                hitboxId: "1",
                damage: "11%",
                angle: "361°",
                kbg: "100",
                bkb: "15",
                skb: "0",
                hitLag: "6",
                shieldStun: "6",
                shieldAdv: "-1",
                size: "4.428"
            },
            {
                hitboxId: "2",
                damage: "8%",
                angle: "361°",
                kbg: "100",
                bkb: "15",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "4.428"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Weak Hit (6-39)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "7%",
                angle: "361°",
                kbg: "100",
                bkb: "10",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "3.6"
            },
            {
                hitboxId: "1",
                damage: "7%",
                angle: "361°",
                kbg: "100",
                bkb: "10",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "3.6"
            },
            {
                hitboxId: "2",
                damage: "7%",
                angle: "361°",
                kbg: "100",
                bkb: "10",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "3.6"
            },
        ]
    },
    {
        name: 'F-air',
        image: 'gifs/aerial/fAir.gif',
        type: 'aerial',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "First Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "13%",
                angle: "361°",
                kbg: "100",
                bkb: "5",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "0",
                size: "4.8"
            },
            {
                hitboxId: "1",
                damage: "13%",
                angle: "361°",
                kbg: "100",
                bkb: "5",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "0",
                size: "2.8"
            },
            {
                hitboxId: "2",
                damage: "13%",
                angle: "361°",
                kbg: "100",
                bkb: "5",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "0",
                size: "2"
            },
            {
                hitboxId: "3",
                damage: "13%",
                angle: "361°",
                kbg: "100",
                bkb: "5",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "0",
                size: "4.4"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Second Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "8%",
                angle: "361°",
                kbg: "90",
                bkb: "0",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "3.6"
            },
            {
                hitboxId: "1",
                damage: "8%",
                angle: "361°",
                kbg: "90",
                bkb: "0",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "2.4"
            },
            {
                hitboxId: "2",
                damage: "8%",
                angle: "361°",
                kbg: "90",
                bkb: "0",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "1.6"
            },
            {
                hitboxId: "3",
                damage: "8%",
                angle: "361°",
                kbg: "90",
                bkb: "0",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "3.6"
            },
        ]
    },
    {
        name: 'U-air',
        image: 'gifs/aerial/uAir.gif',
        type: 'aerial',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Strong Hit (5-7)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "16%",
                angle: "80°",
                kbg: "85",
                bkb: "25",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "-6",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "16%",
                angle: "80°",
                kbg: "85",
                bkb: "25",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "-6",
                size: "4.4"
            },
            {
                hitboxId: "2",
                damage: "16%",
                angle: "80°",
                kbg: "85",
                bkb: "25",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "-6",
                size: "2.4"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Weak Hit (8-49)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "12%",
                angle: "80°",
                kbg: "85",
                bkb: "25",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "-8",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "12%",
                angle: "80°",
                kbg: "85",
                bkb: "25",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "-8",
                size: "4.4"
            },
            {
                hitboxId: "2",
                damage: "12%",
                angle: "80°",
                kbg: "85",
                bkb: "25",
                skb: "0",
                hitLag: "7",
                shieldStun: "7",
                shieldAdv: "-8",
                size: "2.4"
            },
        ]
    },
    {
        name: 'B-air',
        image: 'gifs/aerial/bAir.gif',
        type: 'aerial',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "First Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "7%",
                angle: "361°",
                kbg: "100",
                bkb: "0",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "7%",
                angle: "361°",
                kbg: "100",
                bkb: "0",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "4.4"
            },
            {
                hitboxId: "2",
                damage: "7%",
                angle: "361°",
                kbg: "100",
                bkb: "0",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "5.28"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Second Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "7%",
                angle: "361°",
                kbg: "100",
                bkb: "15",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "7%",
                angle: "361°",
                kbg: "100",
                bkb: "15",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "4.4"
            },
            {
                hitboxId: "2",
                damage: "7%",
                angle: "361°",
                kbg: "100",
                bkb: "15",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: "-2",
                size: "4.4"
            },
        ]
    },
    {
        name: 'D-air',
        image: 'gifs/aerial/dAir.gif',
        type: 'aerial',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Strong Hit (13-15)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "22%",
                angle: "65°",
                kbg: "80",
                bkb: "50",
                skb: "0",
                hitLag: "10",
                shieldStun: "11",
                shieldAdv: "-14",
                size: "4"
            },
            {
                hitboxId: "1",
                damage: "20%",
                angle: "65°",
                kbg: "80",
                bkb: "40",
                skb: "0",
                hitLag: "9",
                shieldStun: "10",
                shieldAdv: "-15",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "20%",
                angle: "65°",
                kbg: "80",
                bkb: "40",
                skb: "0",
                hitLag: "9",
                shieldStun: "10",
                shieldAdv: "-15",
                size: "2.4"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Weak Hit (16-64)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "19%",
                angle: "65°",
                kbg: "80",
                bkb: "50",
                skb: "0",
                hitLag: "9",
                shieldStun: "11",
                shieldAdv: "-15",
                size: "4"
            },
            {
                hitboxId: "1",
                damage: "19%",
                angle: "65°",
                kbg: "80",
                bkb: "40",
                skb: "0",
                hitLag: "9",
                shieldStun: "10",
                shieldAdv: "-15",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "20%",
                angle: "65°",
                kbg: "80",
                bkb: "40",
                skb: "0",
                hitLag: "9",
                shieldStun: "10",
                shieldAdv: "-15",
                size: "2.4"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Bounce",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "6%",
                angle: "65°",
                kbg: "",
                bkb: "",
                skb: "0",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: "4.4"
            },
            {
                hitboxId: "1",
                damage: "6%",
                angle: "65°",
                kbg: "",
                bkb: "",
                skb: "0",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "6%",
                angle: "65°",
                kbg: "",
                bkb: "",
                skb: "0",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: "2.4"
            },
        ]
    },
    {
        name: 'Spin Attack Grounded',
        image: 'gifs/spcl/spinAttk/upBgrounded.gif',
        type: 'specials',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Strong Hit 8-13",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "15%",
                angle: "361°",
                kbg: "95",
                bkb: "32",
                skb: "0",
                hitLag: "8",
                shieldStun: "8",
                shieldAdv: ":(",
                size: "4"
            },
            {
                hitboxId: "1",
                damage: "15%",
                angle: "361°",
                kbg: "95",
                bkb: "32",
                skb: "0",
                hitLag: "8",
                shieldStun: "8",
                shieldAdv: ":(",
                size: "4"
            },
            {
                hitboxId: "2",
                damage: "15%",
                angle: "361°",
                kbg: "95",
                bkb: "32",
                skb: "0",
                hitLag: "8",
                shieldStun: "8",
                shieldAdv: ":(",
                size: "2"
            },
            {
                hitboxId: "3",
                damage: "15%",
                angle: "361°",
                kbg: "95",
                bkb: "32",
                skb: "0",
                hitLag: "8",
                shieldStun: "8",
                shieldAdv: ":(",
                size: "1.2"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Second Hit",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "5%",
                angle: "0°",
                kbg: "100",
                bkb: "60",
                skb: "0",
                hitLag: "4",
                shieldStun: "4",
                shieldAdv: ":(",
                size: "4"
            },
            {
                hitboxId: "1",
                damage: "5%",
                angle: "0°",
                kbg: "100",
                bkb: "60",
                skb: "0",
                hitLag: "4",
                shieldStun: "4",
                shieldAdv: ":(",
                size: "4"
            },
            {
                hitboxId: "2",
                damage: "5%",
                angle: "0°",
                kbg: "100",
                bkb: "60",
                skb: "0",
                hitLag: "4",
                shieldStun: "4",
                shieldAdv: ":(",
                size: "2"
            },
            {
                hitboxId: "3",
                damage: "5%",
                angle: "361°",
                kbg: "95",
                bkb: "32",
                skb: "0",
                hitLag: "5",
                shieldStun: "5",
                shieldAdv: ":(",
                size: "1.2"
            },
        ]
    },
    {
        name: 'Spin Attack Aerial',
        image: 'gifs/spcl/spinAttk/upBaerial.gif',
        type: 'specials',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Hit 1",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "4%",
                angle: "90°",
                kbg: "100",
                bkb: "0",
                skb: "72",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "1",
                damage: "6%",
                angle: "86°",
                kbg: "100",
                bkb: "0",
                skb: "68",
                hitLag: "5",
                shieldStun: "4",
                shieldAdv: ":(",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "6%",
                angle: "80°",
                kbg: "100",
                bkb: "0",
                skb: "68",
                hitLag: "5",
                shieldStun: "4",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "3",
                damage: "6%",
                angle: "75°",
                kbg: "100",
                bkb: "0",
                skb: "68",
                hitLag: "5",
                shieldStun: "4",
                shieldAdv: ":(",
                size: "3.6"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Hit 2",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "2%",
                angle: "90°",
                kbg: "100",
                bkb: "0",
                skb: "64",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "1",
                damage: "2%",
                angle: "86°",
                kbg: "100",
                bkb: "0",
                skb: "60",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "6%",
                angle: "80°",
                kbg: "100",
                bkb: "0",
                skb: "60",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "3",
                damage: "6%",
                angle: "75°",
                kbg: "100",
                bkb: "0",
                skb: "60",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "3.6"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Hit 3",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "2%",
                angle: "90°",
                kbg: "100",
                bkb: "0",
                skb: "58",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "1",
                damage: "2%",
                angle: "86°",
                kbg: "100",
                bkb: "0",
                skb: "55",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "6%",
                angle: "80°",
                kbg: "100",
                bkb: "0",
                skb: "55",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "3",
                damage: "6%",
                angle: "75°",
                kbg: "100",
                bkb: "0",
                skb: "55",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "3.6"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Hit 4",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "2%",
                angle: "90°",
                kbg: "100",
                bkb: "0",
                skb: "46",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "1",
                damage: "2%",
                angle: "86°",
                kbg: "100",
                bkb: "0",
                skb: "43",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "6%",
                angle: "80°",
                kbg: "100",
                bkb: "0",
                skb: "43",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "3",
                damage: "6%",
                angle: "75°",
                kbg: "100",
                bkb: "0",
                skb: "43",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: ":(",
                size: "3.6"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Hit 5",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "4%",
                angle: "360°",
                kbg: "160",
                bkb: "40",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "1",
                damage: "6%",
                angle: "360°",
                kbg: "160",
                bkb: "40",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: ":(",
                size: "4.8"
            },
            {
                hitboxId: "2",
                damage: "6%",
                angle: "360°",
                kbg: "160",
                bkb: "40",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: ":(",
                size: "5.6"
            },
            {
                hitboxId: "3",
                damage: "6%",
                angle: "360°",
                kbg: "160",
                bkb: "40",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: ":(",
                size: "3.6"
            },
        ]
    },
    {
        name: 'Bomb Pull',
        image: 'gifs/spcl/bmb/bmbPull.gif',
        type: 'Special',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "N/A",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
        ]
    },
    {
        name: 'Bomb Throw Forward',
        image: 'gifs/spcl/bmb/bmbThrowF.gif',
        type: 'Special',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Smash Impact",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "4.656%",
                angle: "70°",
                kbg: "60",
                bkb: "60",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "N/A",
                size: "3.752"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Tilt Impact",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "6.656%",
                angle: "70°",
                kbg: "60",
                bkb: "60",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "N/A",
                size: "3.752"
            },
        ]
    },
    {
        name: 'Boomerang Smash Throw',
        image: 'gifs/spcl/boom/boomSmashThrow.gif',
        type: 'Special',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "frame 1",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "16.9%",
                angle: "70°",
                kbg: "30",
                bkb: "55",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "N/A",
                size: "4.8"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "frame 2",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "16.759%",
                angle: "70°",
                kbg: "30",
                bkb: "55",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "N/A",
                size: "4.8"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "frame 3",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "16.618%",
                angle: "70°",
                kbg: "30",
                bkb: "55",
                skb: "0",
                hitLag: "8",
                shieldStun: "9",
                shieldAdv: "N/A",
                size: "4.8"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Late (4-59)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "6%",
                angle: "65°",
                kbg: "30",
                bkb: "55",
                skb: "0",
                hitLag: "5",
                shieldStun: "4",
                shieldAdv: "N/A",
                size: "3.2"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Return (60-171)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "3%",
                angle: "70°",
                kbg: "26",
                bkb: "25",
                skb: "0",
                hitLag: "5",
                shieldStun: "4",
                shieldAdv: "N/A",
                size: "2.8"
            },
        ]
    },
    {
        name: 'Boomerang Tilt Throw',
        image: 'gifs/spcl/boom/boomTiltThrow.gif',
        type: 'Special',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "frame 1",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "14.8ish%",
                angle: "70°",
                kbg: "30",
                bkb: "55",
                skb: "0",
                hitLag: "7",
                shieldStun: "8",
                shieldAdv: "N/A",
                size: "4.8"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "frame 2",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "14.8ish%",
                angle: "70°",
                kbg: "30",
                bkb: "55",
                skb: "0",
                hitLag: "7",
                shieldStun: "8",
                shieldAdv: "N/A",
                size: "4.8"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "frame 3",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "14.8ish%",
                angle: "70°",
                kbg: "30",
                bkb: "55",
                skb: "0",
                hitLag: "7",
                shieldStun: "8",
                shieldAdv: "N/A",
                size: "4.8"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Late (4-44)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "6%",
                angle: "65°",
                kbg: "30",
                bkb: "55",
                skb: "0",
                hitLag: "5",
                shieldStun: "4",
                shieldAdv: "N/A",
                size: "3.2"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Return (45-141)",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "3%",
                angle: "70°",
                kbg: "26",
                bkb: "25",
                skb: "0",
                hitLag: "4",
                shieldStun: "3",
                shieldAdv: "N/A",
                size: "2.8"
            },
        ]
    },
    {
        name: 'Arrow',
        image: 'gifs/spcl/arr/arr.gif',
        type: 'Special',
        stats: [
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Point Blank",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "1%",
                angle: "70°",
                kbg: "50",
                bkb: "8",
                skb: "0",
                hitLag: "3",
                shieldStun: "2",
                shieldAdv: "N/A",
                size: "1.024"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "No Charge",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "5%",
                angle: "70°",
                kbg: "50",
                bkb: "8",
                skb: "0",
                hitLag: "4",
                shieldStun: "4",
                shieldAdv: "N/A",
                size: "1.024"
            },
            {
                hitboxId: "",
                damage: "",
                angle: "",
                kbg: "",
                bkb: "",
                skb: "Max Charge",
                hitLag: "",
                shieldStun: "",
                shieldAdv: "",
                size: ""
            },
            {
                hitboxId: "0",
                damage: "18%",
                angle: "70°",
                kbg: "50",
                bkb: "8",
                skb: "0",
                hitLag: "9",
                shieldStun: "10",
                shieldAdv: "N/A",
                size: "1.024"
            },
        ]
    }

];

var labels = {
    hitboxId: "Hitbox Id",
    damage: "Damage",
    angle: "Angle",
    kbg: "KBG",
    bkb: "BKB",
    skb: "SKB",
    hitLag: "Hit Lag",
    shieldStun: "Shield Stun",
    shieldAdv: "Shield Adv",
    size: "Size"
};



// hitboxId:
// damage:
// angle:
// kbg:
// bkb:
// skb:
// hitLag:
// shieldStun:
// shieldAdv:
// size:
// var uTilt = [
    //     ["0", "9%", "95°", "122", "30", "0", "6", "6", "-14/-9", "3.6"],
    //     ["1", "9%", "85°", "130", "30", "0", "6", "6", "-14/-9", "4.2"],
    //     ["2", "9%", "85°", "124", "30", "0", "6", "6", "-14/-9", "2.8"],
    //     ["3", "9%", "85°", "123", "30", "0", "6", "6", "-14/-9", "2.8"]
// ];

var uSmashLabels = ["Hitbox Id", "damage", "angle", "kbg", "bkb", "skb", "Hit Lag", "Shield Stun", "Shield Adv", "size"];
var uSmash = [
    // first hit
    // ["0", "4", "98°", "100", "40", "26", "4", "3", "-", "3.2"],
    // ["1", "4", "98°", "100", "40", "30", "4", "3", "-", "3.2"],
    // ["2", "4", "98°", "100", "40", "33", "4", "3", "-", "3.2"],
    // ["3", "4", "98°", "100", "40", "33", "4", "3", "-", "2"],
    // second hit
    // ["Hitbox Id", "damage", "angle", "kbg", "bkb", "skb", "Hit Lag", "Shield Stun", "Shield Adv", "size"],
    // ["0", "2", "90°", "100", "0", "30", "3", "2", "-", "3.2"],
    // ["1", "2", "90°", "100", "0", "33", "3", "2", "-", "3.2"],
    // ["2", "2", "90°", "100", "0", "37", "3", "2", "-", "3.2"],
    // ["3", "2", "90°", "100", "0", "40", "3", "2", "-", "2"],
    // third hit
    // ["Hitbox Id", "damage", "angle", "kbg", "bkb", "skb", "Hit Lag", "Shield Stun", "Shield Adv", "size"],
    // ["0", "10%", "90°", "80", "90", "0", "6", "6", "-4/-2", "3.2"],
    // ["1", "10%", "90°", "80", "90", "0", "6", "6", "-4/-2", "3.6"],
    // ["2", "9%", "90°", "80", "90", "0", "6", "6", "-4/-2", "3.2"],
    // ["3", "9%", "90°", "80", "90", "0", "6", "6", "-4/-2", "2"]
];

// var rapidJabLabels = ["Hitbox Id", "damage", "angle", "kbg", "bkb", "skb", "Hit Lag", "Shield Stun", "Shield Adv", "size"];
// var rapidJab = [
//     ["0", "1%", "0°", "40", "15", "0", "3", "2", "-10", "4.4"],
//     ["1", "1%", "0°", "40", "15", "0", "3", "2", "-10", "4.4"],
//     ["2", "1%", "0°", "40", "15", "0", "3", "2", "-10", "2.8"],
//     ["3", "1%", "0°", "40", "15", "0", "3", "2", "-10", "2"]
// ];

// var jab2Labels = ["Hitbox Id", "damage", "angle", "kbg", "bkb", "skb", "Hit Lag", "Shield Stun", "Shield Adv", "size"];
// var jab2 = [
//     ["0", "3%", "361°", "60", "10", "0", "4", "3", "-7", "4.4"],
//     ["0", "3%", "361°", "60", "10", "0", "4", "3", "-7", "4"],
//     ["0", "3%", "361°", "60", "10", "0", "4", "3", "-7", "2.4"],
//     ["0", "3%", "361°", "60", "10", "0", "4", "3", "-7", "2.4"]
// ];


// var jab1Labels = ["hitboxId", "damage", "angle", "kbg", "bkb", "skb", "Hit Lag", "Shield Stun", "Shield Adv", "size"];
// var jab1 = [
//     ['0', '5%', '361°', '60', '10', '0', '4', '4', '-9', '4.4'],
//     ["1", "5%", "361°", "60", "10", "0", "4", "4", "-9", "4"], 
//     ["2", "5%", "361°", "60", "10", "0", "4", "4", "-9", "2.4"],
//     ["3", "5%", "361°", "60", "10", "0", "4", "4", "-9", "2.4"]
// ];

$('#normal, #aerial, #special1, #special2, #special3, #special4').on('click', '.button', function (event) {
    event.preventDefault();
    console.log($(this).attr('data-id'));
    var id = $(this).attr('data-id');
    buildTable(id, labels);
});


function buildTable(id, labels) {
    var table = document.createElement('table');
    var thead = document.createElement('thead');
    var tbody = document.createElement('tbody');
    var container = document.getElementById('table-container');
    var newImg = document.createElement('img');
    newImg.setAttribute('src', attacks[id].image);
    

    var theadTr = document.createElement('tr');
    
    for(var prop in attacks[id].stats[0]) {
        var theadTh = document.createElement('th');
        theadTh.innerHTML = labels[prop];
        theadTr.appendChild(theadTh);
    }


    thead.appendChild(theadTr);
    table.appendChild(thead);
    attacks[id].stats.forEach(function(attack) {
        var tbodyTr = document.createElement('tr');
        for (var prop in attack) {
            console.log(attack[prop]);
            var tbodyTd = document.createElement('td');
            tbodyTd.innerHTML = attack[prop];
            tbodyTr.appendChild(tbodyTd);
        }
        tbody.appendChild(tbodyTr);    
    })

    // for (var a = 0; a < data.length; a++) {
    
    // }
    table.appendChild(tbody);

    
    container.innerHTML = "";
    var h1 = document.createElement('h1');
    h1.textContent = attacks[id].name;
    container.append(h1);
    container.append(newImg);
    container.append(table);
}

var Link = new Object();
Link.row0 = ["Weight", "Walk Speed", "Run Speed", "Air Speed", "Air Acceleration", "Fall Speed", "FF Speed", "Gravity", "Traction", "Jump Squat", "SH Air Time", "FH Air Time", "SH FF Air Time", "FH FF Air Time", "Earliest SH FF", "Earliest FH FF"];
Link.row1 = ["104", "1.2", "1.3", "1", ".06", "2.13", "3", ".11", ".1", "6 Frames", "32 Frames", "49 Frames", "20 Frames", "36 Frames", "Frame 14", "Frame 23"];
Link.row2 = ["6-7th", "6-7th", "23-24th", "6-7th", "5-11th", "7-9th", "4-5th", "8-11th", "1-4th", "23-25th", "N/A", "N/A", "N/A", "N/A", "N/A", "N/A",];